...
def test_validate_stack():
    ...
    # empty grid
    e = EdgeSet(2,2)
    assert e.validate_stacks()  # empty grids are always valid
    ...
    assert not e.validate_stacks()  # missing horizontal edges below (0, 2)
    ...
    assert e.is_down_ok(0, 2)
    assert e.is_down_ok(1, 1)
    assert not e.validate_stacks()  # stacks are ok, but square constraint is not for upper-left square
    ...
    assert e.validate_stacks()  # fixed!
